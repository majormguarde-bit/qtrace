...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if tenant and tenant.subscription_plan:
            ...
            if current_user_count >= tenant.subscription_plan.max_users:
                ...
                raise PermissionDenied(f"Превышен лимит пользователей для вашего тарифа ({tenant.subscription_plan.max_users}).")
        serializer.save()
    ...
    def create_employee(self, request):
        ...
        if tenant and tenant.subscription_plan:
            ...
            if current_user_count >= tenant.subscription_plan.max_users:
                return Response(
                    {'error': f'Превышен лимит пользователей для вашего тарифа ({tenant.subscription_plan.max_users}).'}, 
