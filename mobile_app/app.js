        ...
        <div class="col-12">
            <div class="card task-card border-0 shadow-sm p-3" onclick="showTaskDetail(${JSON.stringify(task).replace(/"/g, '&quot;')})">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    ...
                    <small class="text-muted fw-bold">${task.external_id || '---'}</small>
                </div>
                <h6 class="fw-bold mb-1">${task.title}</h6>
                <div class="d-flex gap-2 mb-2">
                    <span class="badge bg-secondary-soft text-dark border ultra-small">P${task.priority}</span>
                    <span class="badge bg-light text-muted border ultra-small">${task.process_type}</span>
                </div>
                <p class="text-muted small mb-0 text-truncate">${task.description || 'Нет описания'}</p>
            </div>
        </div>
    `).join('');
}
...
function showTaskDetail(task) {
    ...
    document.getElementById('taskDetailContent').innerHTML = `
        ...
        <div class="small text-muted mb-2">ID: ${task.external_id || task.id} | Тип: ${task.process_type || '---'}</div>
        <p class="text-muted mb-4">${task.description || 'Описание отсутствует'}</p>
        
        <div class="mb-4">
            <h6 class="fw-bold mb-3 small text-muted text-uppercase">Аналитика</h6>
            <div class="row g-2 mb-3">
                <div class="col-6">
                    <div class="p-2 border rounded bg-light text-center">
                        <div class="ultra-small text-muted">Cycle Time</div>
                        <div class="fw-bold small">${task.cycle_time || 0} м</div>
                    </div>
                </div>
                <div class="col-6">
                    <div class="p-2 border rounded bg-light text-center">
                        <div class="ultra-small text-muted">Efficiency</div>
                        <div class="fw-bold small">${task.efficiency_score || 0}</div>
                    </div>
                </div>
            </div>
            <h6 class="fw-bold mb-3 small text-muted text-uppercase">Этапы задачи</h6>
            <div class="list-group list-group-flush bg-transparent">
                ${(task.stages || []).map(stage => `
                    <div class="list-group-item bg-transparent d-flex justify-content-between align-items-center px-0 border-light">
                        ...
                        <span class="badge rounded-pill bg-light text-dark border small">${stage.planned_duration} мин</span>
                    </div>
